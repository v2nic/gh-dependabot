...
// Run submits dependencies from a lockfile to the dependency graph API
func Run(repoFlag, lockfileFlag string, dryRun bool) error {
	// Determine repo
	owner, repoName, err := resolveRepo(repoFlag)
	if err != nil {
		return fmt.Errorf("resolve repo: %w", err)
	}

	// Find lockfile
	lockfilePath, err := findLockfile(lockfileFlag)
	if err != nil {
...
func convertDeps(deps map[string]types.Dependency) map[string]submitter.Dependency {
	result := make(map[string]submitter.Dependency, len(deps))
	for k, v := range deps {
		result[k] = submitter.Dependency{
